...
public class ImageBrowsePresenter {
    ...
    public void saveImage(){
        ...
        Picasso.with(view.getMyContext()).load(NetConstantUrl.IMAGE_URL+imageUrl).into(new Target() {
            @Override
            public void onBitmapLoaded(Bitmap bitmap, Picasso.LoadedFrom from) {
                // 创建目录
                File appDir = new File(Environment.getExternalStorageDirectory(), "hyx_Image");
                if (!appDir.exists()) {
                    appDir.mkdir();
                }
